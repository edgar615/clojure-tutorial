...
;头保持
;一旦惰性序列的一个元素被实例化之后，只要你保持了对这个序列的引用，那么这个元素就会一直保持着了。
;这意味着，只要你保持了对序列的一个引用，那么序列中的元素旧不能被垃圾回收。这种问题称为“头保持”，会给虚拟机压力从而影响性能，如果序列中的元素占用内存过大的话甚至可能盗跖内存用尽的错误。

;https://xumingming.sinaapp.com/977/clojure-lazyseq-head-retention/
